...
public sealed class FigletText : Renderable, IHasJustification
{
    ...
    protected override IEnumerable<Segment> Render(RenderOptions options, int maxWidth)
    {
        ...
        foreach (var row in GetRows(maxWidth))
        {
                ...
                && row.Count > 1)
            {
                ...
                {
                    junctions[i] = ComputeJunction(row[i], row[i + 1], LayoutMode, smushRules, _font.Hardblank);
                }
            }
            ...
            for (var index = 0; index < _font.Height; index++)
            {
                var lineText = junctions != null
                    ? BuildLine(row, index, junctions, _font.Hardblank)
                    : string.Concat(row.Select(x => x.Lines[index]));

                // Replace hardblanks with regular spaces for display
                if (_font.Hardblank != ' ')
                {
                    lineText = lineText.Replace(_font.Hardblank, ' ');
                }
        ...
        FigletLayoutMode mode, int smushRules, char hardblank)
    {
        ...
        // Per the Figlet spec, hardblanks count as blanks for overlap distance,
        // so replace them with spaces before trimming.
        var fit = int.MaxValue;
        for (var y = 0; y < left.Height; y++)
        {
            var leftNorm = left.Lines[y].Replace(hardblank, ' ');
            var rightNorm = right.Lines[y].Replace(hardblank, ' ');
            var trailing = left.Lines[y].Length - leftNorm.TrimEnd(' ').Length;
            var leading = right.Lines[y].Length - rightNorm.TrimStart(' ').Length;
        ...
        for (var i = 0; i < left.Height; i++)
        {
            ...
            var trailing = leftLine.Length - leftLine.Replace(hardblank, ' ').TrimEnd(' ').Length;
            ...
            var merged = SmushChars(leftChar, rightChar, smushRules, hardblank);
            if (merged == null)
            {
                return (fit, null);
            }
    ...
    private static char? SmushChars(char left, char right, int rules, char hardblank)
    {
        switch (left)
        {
        ...
        {
            return rules == 0 || (rules & 32) != 0 ? hardblank : (char?)null;
        }

        // A hardblank at the smushing boundary was already within the fitting/overlap
        // zone (counted as space for distance purposes), so treat it as transparent here
        if (left == hardblank)
        {
            left = ' ';
        }
        ...
        char hardblank)
    {
        ...
        for (var rowIndex = 1; rowIndex < row.Count; rowIndex++)
        {
            ...
            var trailingSpaces = sb.CountTrailing([' ', hardblank]);
            ...
            if (mergeChars != null)
            {
                ...
                // Remove trailing spaces and boundary char from accumulated left
                ...
                // Skip leading spaces and boundary char from right
